...
class _SubscriptionPageState extends State<SubscriptionPage> {
  ...
  // dynamicCard
  // CHANGED:
  //  - padding is now EdgeInsets.all(...) — same on every side, on every
  //    card (monthly no longer gets a special-cased larger top padding).
  //  - the whole inner Column is wrapped in a LayoutBuilder + FittedBox
  //    (fit: BoxFit.scaleDown). If the content (big number + title + price
  //    + benefits list) would overflow the fixed card height on a small
  //    phone, it now scales down as a whole instead of clipping/overflowing.
  // ---------------------------------------------------------------------
  Widget dynamicCard({
    required String id,
    ...
    final bool isMonthly = (id == "monthly"); // Monthly card ke liye special check
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          padding: EdgeInsets.symmetric(
            ...
            vertical: isMonthly ? AppSize.h(12) : AppSize.h(14),
          ),
          decoration: BoxDecoration(
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  SizedBox(height: isMonthly ? AppSize.h(4) : AppSize.h(2)), // Monthly me thoda extra gap
                  Center(
                    child: Text(
                      title,
                      ...
                      maxLines: 2,
                      style: TextStyle(
                        fontSize: isCredit ? AppSize.sp(18.0) : AppSize.sp(22.0),
                        height: 1.1,
                        fontWeight: FontWeight.bold,
                        color: Colors.black87,
                      ),
                  ...
                  SizedBox(height: isMonthly ? AppSize.h(4) : AppSize.h(2)),
                  Center(
                    child: Text(
                      price,
                      textAlign: TextAlign.center,
                      maxLines: 1,
                      style: TextStyle(
                        fontSize: isTrial ? AppSize.sp(14.0) : AppSize.sp(16.0),
              ...
              Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  // Monthly card ke benefits section ke upar extra space add karne ke liye
                  if (isMonthly) SizedBox(height: AppSize.h(4)),
                  Row(
                    children: [
                      Text(
                        "Benefits",
                  ...
                  SizedBox(height: isMonthly ? AppSize.h(6) : AppSize.h(4)), // Monthly benefits ke beech ka gap
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              // CHANGED: every Text on the left column is now
                              // wrapped in FittedBox(fit: BoxFit.scaleDown)
                              // instead of relying on maxLines + ellipsis, so
                              // long strings (like the renewal date line)
...
//   double _currentPage = 1.0;
//
//   // Fixed card width (logical px), same treatment as the fixed card height.
//   // Used directly on the card's Container below.
//   static const double _cardWidth = 175.0;
//
//   final List<Map<String, dynamic>> _plans = [
//     {
//       "id": "monthly",
...
//   // dynamicCard
//   // CHANGED:
//   //  - Card height is now AppSize.h(254) (was 235).
//   //  - Card width is controlled by the PageController's viewportFraction
//   //    (see didChangeDependencies) which is derived from the screen width
//   //    to target ~175 logical px — dynamic per device, not a hardcoded
//   //    static width.
//   //  - Padding is uniform on every side, on every card (no isMonthly
//   //    special-case).
//   //  - The old fixed SizedBox gap between the top info block and the
//   //    "Benefits" row is replaced with a Spacer(). Because the outer Column
//   //    now has mainAxisSize: MainAxisSize.max, that Spacer absorbs any
//   //    extra vertical room the taller (254) card has — so the layout fills
//   //    the card evenly instead of leaving a dead empty gap at the bottom.
//   //  - Individual text elements keep FittedBox(fit: BoxFit.scaleDown) so
//   //    nothing gets clipped even in the rare case content is still tight.
//   // ---------------------------------------------------------------------
//   Widget dynamicCard({
//     required String id,
//     required String bigNumber,
...
//         onTap: onTap,
//         child: Container(
//           height: AppSize.h(254),
//           width: AppSize.w(_cardWidth),
//           margin: EdgeInsets.symmetric(horizontal: AppSize.w(2)),
...
//                 ),
//               ),
//               // CHANGED: flexible gap instead of a fixed SizedBox — absorbs
//               // the extra height from the taller card so there's no empty
//               // dead space below the benefits list.
//               const Spacer(),
//               Row(
//                 children: [
//                   Text(
...
//                               ),
//                               SizedBox(height: AppSize.h(14)),
//                               // CHANGED: card viewport height bumped to match
//                               // the new AppSize.h(254) card height, plus a
//                               // little breathing room so the Transform.scale
//                               // on the focused card isn't clipped.
//                               SizedBox(
//                                 height: AppSize.h(254),
//                                 child: PageView.builder(
//                                   controller: _pageController,
...
//
//                               // Available Credits Container Box
//                               // Every Text on the left column is wrapped in
//                               // FittedBox(fit: BoxFit.scaleDown) instead of
//                               // relying on maxLines + ellipsis, so long
//                               // strings (like the renewal date line) shrink
//                               // to fit instead of getting cut off.
//                               Padding(
//                                 padding: EdgeInsets.symmetric(horizontal: AppSize.w(4)),
//                                 child: Center(
//                                   child: Container(
